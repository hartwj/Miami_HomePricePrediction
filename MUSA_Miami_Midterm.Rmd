...
This project attempts to predict housing prices in metropolitan Miami by taking into consideration a home’s unique features (e.g., fence, patio) as well as considering local amenities like schools, parks, and access to major roads. To create our model, we converted our features of interest into variables that can be fed into an OLS regression model. Then we tested each for correlation with home sale prices and fine-tuned our model until we were able to minimize error.  
...
``` {r shore, echo=FALSE, message=FALSE, warning=FALSE}
...
``` {r middle school, echo=FALSE, message=FALSE, warning=FALSE}
### Middle School Areas
...
stargazer(sumstat.df, title="Summary Statistics", results ='asis', message=FALSE, type='html', 
...
```{r K-Fold Cross Validation, message=FALSE, warning=FALSE, echo=FALSE}
...
```{r Outputting our CV predictions to CV, echo=FALSE, message=FALSE, warning=FALSE}
...
``` {r test v. train, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r spatial corr of errors, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r Map of Neighborhood and MAPEs, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r Generalizability of Income, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}
