...
```{r mae table, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
mae_table <- data.frame(Test = c("Test MAE:", "Test MAPE:"), Values = c(reg2MAE, reg2MAPE))
...
```{r cv results, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
...
## Histogram of Cross Validation Errors (MAE)
...
```{r residuals, echo=FALSE, warning=FALSE, message=FALSE}
# Residual Map
ggplot() +
  annotation_map_tile("cartolight") +
  geom_sf(data=miami.base, fill='transparent') + 
  geom_sf(data = miami.test,
          aes(color = SalePrice.Error), size = 1.5) +
  mapTheme() +
  labs(title="Errors by Neighborhood (MAPE)",
       caption = "Map tiles by Carto, under CC BY 3.0. Data by OpenStreetMap, under ODbL.") +
  scale_color_viridis(name = "Map of Errors\n(Residuals)",
                      direction = -1)
...
## Combined Total Sale Prices (Training and Test)
...
```{r Predicted Values when toPredict is 0 and 1, warning=FALSE, echo=FALSE, message=FALSE}
...
ggplot() + 
  ...
  geom_sf(data=allprices.sf, aes(color=SalePrice),
          show.legend = "line", size = 1) + 
  labs(title = "Predicted & Training Set Home Prices",       
       caption = "Map tiles by Carto, under CC BY 3.0. Data by OpenStreetMap, under ODbL.") +
  scale_color_viridis(option="C", 
                      name = "Price ($)", 
                      limits=c(10000,1000000),
...
miami.test$Brownsville.MS <- ifelse(is.na(miami.test$Brownsville.MS),0,miami.test$Brownsville.MS)
miami.test$CitrusGrove.MS <- ifelse(is.na(miami.test$CitrusGrove.MS),0,miami.test$CitrusGrove.MS)
miami.test$JosedeDiego.MS <- ifelse(is.na(miami.test$JosedeDiego.MS),0,miami.test$JosedeDiego.MS)
miami.test$GeorgiaJA.MS <- ifelse(is.na(miami.test$GeorgiaJA.MS),0,miami.test$GeorgiaJA.MS)
miami.test$KinlochPk.MS <- ifelse(is.na(miami.test$KinlochPk.MS),0,miami.test$KinlochPk.MS)
miami.test$Madison.MS <- ifelse(is.na(miami.test$Madison.MS),0,miami.test$Madison.MS)
miami.test$Nautilus.MS <- ifelse(is.na(miami.test$Nautilus.MS),0,miami.test$Nautilus.MS)
miami.test$Shenandoah.MS <- ifelse(is.na(miami.test$Shenandoah.MS),0,miami.test$Shenandoah.MS)
miami.test$WestMiami.MS <- ifelse(is.na(miami.test$WestMiami.MS),0,miami.test$WestMiami.MS)

miami.test_preds <- miami.test %>%
  mutate(Regression = "Baseline Regression",
         SalePrice.Predict = predict(finaltraining, miami.test),
         SalePrice.Error = SalePrice.Predict - SalePrice,
         SalePrice.AbsError = abs(SalePrice.Predict - SalePrice),
...
colors2 <- c('#f1eef6', '#bdc9e1', '#74a9cf', '#2b8cbe', '#045a8d','gray')
  
# MAPE Map
ggplot() +
  annotation_map_tile("cartolight") +
  geom_sf(data=miami.base, fill='transparent') + 
  geom_sf(data = miami.test, aes(color = SalePrice.APE),  size = 2) +
  labs(title = "Errors by Neighborhood (MAPE)",
       caption = "Map tiles by Carto, under CC BY 3.0. Data by OpenStreetMap, under ODbL.") +
  mapTheme() +
  scale_color_viridis(name='MAPE',
                      direction = -1) 
...
````{r Price-MAPE Scatter, echo=FALSE, message=FALSE, warning=FALSE}
miami.test <-
  miami.test %>%
  mutate(Regression = "Baseline Regression",
         SalePrice.Predict = predict(finaltraining, miami.test),
