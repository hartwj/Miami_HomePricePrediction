...
```{r mae/mape, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
The histogram below examines our final cross validation model run on 100 folds. The MAE, or Mean Absolute Error, measures the difference in actual and predicted sales price across each test subset. Our errors clustered between $50k and $75k. 
...
```
Juliana - no clue what these maps are for, so I'll defer to you. Made up this: The map on the left shows our prediction for all home prices in Miami. The map on the right shows all home prices in the training dataset. 
